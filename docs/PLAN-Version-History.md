...
**Lo que enseñó la primera corrida (18 de septiembre).** Cuatro de cinco casos como los issues; el quinto tenía razón la historia: el commit que el autor llama el fix del bug 1 cambia los nombres que el wrapper comprueba, no el directorio donde los busca, y el wrapper sigue saliendo por la misma línea — en ese commit y en el head de la rama. Y dos fallas nuestras que el mismo run destapó (PR #41): los legs de ejecución corrían sin imagen tras un build fallido, y el "pull access denied" de esa corrida inexistente se diagnosticaba como una imagen base que STRhub no pudo bajar; y `unzip: cannot find` no tenía regla. Más una tercera, de diseño: un caso `proposed` del **mismo commit** que tiene resultado curado en el catálogo lo **reemplaza** (una fila por commit, la última verificación) — pasó con `dafdee7`, y hubo que re-publicar la receta curada. De ahí `publish: false` por caso (y el input `publish` del workflow): la corrida se hace, se sostiene contra su expectativa, y no toca el catálogo. Lo que queda abierto es más grande que este check: dos instrumentos sobre el mismo commit son dos hechos, y el historial guarda uno.

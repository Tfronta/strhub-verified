...
INDEX = {"tools": [
    {"slug": "hipstr-autosomal", "source_repo": "https://github.com/tfwillems/HipSTR", "source_ref": VERIFIED},
    {"slug": "hipstr-y", "source_repo": "https://github.com/tfwillems/HipSTR", "source_ref": VERIFIED},
...
def test_finds_newer_release_once_per_repo_and_skips_current_tools():
    ...
    assert slugs == ["hipstr-autosomal", "hipstr-y"]
...
def test_retarget_swaps_ref_in_manifest_and_dockerfile_and_keeps_the_slug():
    r = retarget_recipe.retarget("hipstr-autosomal", NEW)
    # The slug is the tool's identity, not the commit's: a newer ref keeps it.
    assert r["slug"] == "hipstr-autosomal"
    manifest = r["recipe"]["manifest_yml"]
    assert f'ref: "{NEW}"' in manifest and VERIFIED not in manifest
    assert 'slug: "hipstr-autosomal"' in manifest
...
def test_dispatch_is_a_trial_with_the_retargeted_recipe():
    ...
    assert any(a.startswith("tool=hipstr-autosomal") for a in calls[0])
    body = ur.issue_body(done)
    assert "hipstr-autosomal" in body and "v0.8" in body and "trial" in body
